...
retrospective_review 模式必须把正文与 review_context 中的最小记录集合逐项对照，不能因为存在 [R-*] 格式就假定来源支持判断；还必须逐项决定 profile_entries 是否 accepted 或 rejected：只有记录直接支持、相对稳定、值得跨周期保留的观点、理念、理想、行为模式和关注领域才能接受；topic_decisions 返回空数组。
research_review 模式逐项检查每个 research_topics：外部来源是否真正支持正文，是否包含反例或边界，是否把探索性推断明确标为推断，以及是否避免替用户做最终判断。对每个输入 topic_id 返回 accepted 或 rejected；证据薄弱或正文越界时拒绝该主题，不要为了整份报告凑齐主题。entry_decisions 返回空数组。
...
只返回 JSON：{"pass":true或false,"entry_decisions":[{"temp_id":"p1","status":"accepted|rejected","reason":"..."}],"topic_decisions":[{"topic_id":"Q001","status":"accepted|rejected","reason":"..."}],"unsupported_claims":["..."],"required_changes":["..."],"summary":"..."}。""",
)
...
def validate(
    ...
    *,
...
) -> tuple[bool, dict[str, str], dict[str, str], list[str]]:
    ...
    return payload["pass"], normalized, normalized_topics, feedback
