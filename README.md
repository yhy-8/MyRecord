...
原始 Markdown 日记是唯一事实源。SQLite 只保存可丢弃的报告运行状态和失败重试阶段缓存；报告正文已经包含记录依据、使用模型、生成耗时和 Token 用量，阅读不依赖数据库。进一步了解项目时，可分别阅读[设计原则与系统架构](./Docs/设计原则与系统架构.md)、[Agent 与报告流程](./Docs/Agent与报告流程.md)和[运行机制与数据](./Docs/运行机制与数据.md)。
...
配置项、默认值和注释见应用目录中的 [`config.yaml`](./config.yaml)。初次使用需填写模型密钥并确认 `current_model`；启用周报时还必须配置 `third_search`，月报不联网。`retry` 统一控制 Agent 修订、日记总结补写、HTTP/空响应重试、自动暂停阈值和等待间隔；其中 `*_retry_limit` 均不含首次调用。相对目录以配置文件所在目录为基准。DeepSeek V4 默认启用 JSON Output，并把 `max_tokens` 设为 `100000`，只作为接口异常输出与成本的安全预算。语义正文不做生成后的字数判定，提示词只要求在完整前提下简洁。
...
同一输入的内容、JSON 或审查失败不会无限重跑。四个报告 Agent 使用 JSON 模式返回最小对象；无法解析的 JSON 直接结束当前阶段，已解析结果的字段/正文校验或 Reviewer 拒绝共享 `retry.agent_revision_limit` 次修订机会。自动任务在同一输入累计失败达到 `retry.automation_content_failure_limit` 后暂停；日记输入、模型、重试策略或周报搜索配置变化后会自动解锁，也可随时用 `/retry` 明确重试。同输入的周报重试会复用已经完成的安全阶段和搜索证据。其他次数与等待时间均读取 `retry` 配置，不在代码中另设固定值。
...
SQLite 只是缓存；需要全新运行状态时可以在没有报告进程的情况下删除数据库及同名 `-wal`、`-shm`。
...
周报和月报文件头会直接显示模型、生成耗时及输入、输出、缓存命中和总 Token。
