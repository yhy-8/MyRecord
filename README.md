...
配置项、默认值和注释见应用目录中的 [`config.yaml`](./config.yaml)。初次使用需填写模型密钥并确认 `current_model`；启用周报时还必须配置 `third_search`，月报不联网。相对目录以配置文件所在目录为基准。DeepSeek V4 默认启用 JSON Output，并把单次生成上限设为 `100000`；这不是上下文窗口，Agent 自身仍有更小的正文和输入限制。
