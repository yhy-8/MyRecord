...
| 层级 | 文档 | 内容 |
|---|---|---|
| **设计基线（唯一权威）** | [`Docs/设计基线.md`](./Docs/设计基线.md) | 整合全部设计：架构、同步、鉴权/TLS、存储、日志、云端 AI 分析（单次 Report Agent + [N] 引用与来源表）、自动化 |
...
| 历史/深度参考 | [`Docs/代码结构.md`](./Docs/代码结构.md)、[`Docs/安全、通信与日志.md`](./Docs/安全、通信与日志.md) | 结构与安全细节；与基线冲突处以基线为准 |
...
- [`Docs/设计基线.md`](./Docs/设计基线.md) — **唯一权威设计基线**：整合所有设计（含单次 Report Agent、[N] 引用与来源表、无 AI 审核的报告生成）
- [`Docs/代码结构.md`](./Docs/代码结构.md) — 历史结构说明（代码模块职责、数据流、entry_id、同步协议）
- [`Docs/部署指南.md`](./Docs/部署指南.md) — 简明部署步骤（服务端 + 凭证 + TLS + 客户端 + 备份）
- [`Docs/安全、通信与日志.md`](./Docs/安全、通信与日志.md) — 历史安全说明（强制加密、连接凭证、服务端日志内容）
