...
配置项、默认值和注释见应用目录中的 [`config.yaml`](./config.yaml)。初次使用需填写模型密钥并确认 `current_model`；启用周报时还必须配置 `third_search`，月报不联网。相对目录以配置文件所在目录为基准。DeepSeek V4 默认启用 JSON Output，并把 `max_tokens` 设为 `100000`，只作为接口异常输出与成本的安全预算。语义正文不做生成后的字数判定，提示词只要求在完整前提下简洁。
...
同一输入的内容、JSON 或审查失败不会无限重跑：日总结、周报和月报最多自动执行首次尝试和一次整点重试。四个报告 Agent 使用 JSON 模式返回最小对象；无法解析的 JSON 直接结束当前阶段，已解析结果的字段/正文校验或 Reviewer 拒绝共享最多一次修订机会。日记输入、模型或周报搜索配置变化后会自动解锁，也可随时用 `/retry` 明确重试；同输入的周报重试会复用已经完成的安全阶段和搜索证据。接口返回 `stop` 但最终正文为空时，客户端会立即请求一次完整补答。网络和限流故障仍按 5 分钟重试。
