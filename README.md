...
原始 Markdown 日记是唯一事实源。SQLite 只保存可丢弃的报告运行状态和失败重试阶段缓存；报告正文已经包含来源索引、使用模型、生成耗时和 Token 用量，阅读不依赖数据库。进一步了解项目时，可分别阅读[设计原则与系统架构](./Docs/设计原则与系统架构.md)、[Agent 与报告流程](./Docs/Agent与报告流程.md)和[运行机制与数据](./Docs/运行机制与数据.md)。
