...
同一输入的内容、JSON 或审查失败不会无限重跑：日总结、周报和月报最多自动执行首次尝试和一次整点重试；报告生成阶段内部把结构修复与实质内容修订分开计数，最多两次结构修复和一次 Reviewer 内容修订。日记输入、模型或周报搜索配置变化后会自动解锁，也可随时用 `/retry` 明确重试；同输入的周报重试会复用已经完成的安全阶段和搜索证据。接口返回 `stop` 但最终正文为空时，客户端会立即请求一次完整补答。网络和限流故障仍按 5 分钟重试。
